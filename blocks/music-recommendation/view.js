document.addEventListener( 'click', ( event ) => {
	...
	if ( ! audio ) {
		...
		audio.addEventListener( 'ended', () => {
			...
			button.setAttribute(
				'aria-label',
				button.dataset.playLabel || 'Hörprobe abspielen'
			);
		} );
	}
	...
	const setPlaying = ( isPlaying ) => {
		...
		button.setAttribute(
			'aria-label',
			isPlaying
				? button.dataset.pauseLabel || 'Hörprobe pausieren'
				: button.dataset.playLabel || 'Hörprobe abspielen'
		);
	};
	...
	document
		.querySelectorAll( '.child-music-card__preview-button.is-playing' )
		.forEach( ( playingButton ) => {
			...
			playingButton.setAttribute(
				'aria-label',
				playingButton.dataset.playLabel || 'Hörprobe abspielen'
			);
		} );
