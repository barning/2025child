...
function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: 'child-media-cover-grid-block',
	} );
	const enabledTypes = Array.isArray( mediaTypes ) ? mediaTypes : [];
	const previewItems = PREVIEW_ITEMS.filter( ( item ) =>
	...
	return (
		<div { ...blockProps }>
			<InspectorControls>
				...
				>
					<SelectControl
						...
						options={ [
							{
								label: __( 'Neueste Beiträge zuerst', 'child' ),
								value: 'newest',
							},
							{
								label: __( 'Älteste Beiträge zuerst', 'child' ),
								value: 'oldest',
							},
							{
								label: __( 'Titel A–Z', 'child' ),
								value: 'title',
							},
						] }
						onChange={ ( value ) =>
					...
					<SelectControl
						...
						options={ [
							{
								label: __( 'Zum Beitrag', 'child' ),
								value: 'post',
							},
							{
								label: __(
									'Zum externen Medien-Link',
									'child'
								),
								value: 'external',
							},
							{
								label: __( 'Kein Link', 'child' ),
								value: 'none',
							},
						] }
						onChange={ ( value ) =>
					...
					<ToggleControl
						label={ __(
							'Doppelte Erwähnungen einzeln anzeigen',
							'child'
						) }
						help={ __(
							'Ausgeschaltet: dasselbe Medium erscheint nur einmal, auch wenn es in mehreren Beiträgen erwähnt wird.',
							'child'
						) }
						checked={ allowDuplicates }
						onChange={ ( value ) =>
							setAttributes( { allowDuplicates: value } )
						}
					/>
				</PanelBody>
			</InspectorControls>
			...
			<div className="child-media-cover-grid__editor-note">
				{ __(
					'Dieses Grid wird auf der Website automatisch aus veröffentlichten Beiträgen befüllt.',
					'child'
				) }
			</div>
			<div className="child-media-cover-grid">
				{ previewItems.length ? (
					previewItems.map( ( item ) => (
						<div
							key={ item.type }
							className={ `child-media-cover-grid__item child-media-cover-grid__item--${ item.type }` }
						>
							<div
								className={ `child-media-cover-grid__cover child-media-cover-grid__cover--${ item.type } child-media-cover-grid__cover--${ item.coverFormat }` }
								aria-hidden="true"
							>
					...
					<p className="child-media-cover-grid__empty">
						{ __(
							'Wähle mindestens einen Medientyp aus.',
							'child'
						) }
					</p>
				) }
